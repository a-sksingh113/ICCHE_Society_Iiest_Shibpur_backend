...
const handleAddVolunteer = async (req, res) => {
    try {
      const { fullName, email, contactNumber, enrollmentNo, gender, year, department, residenceType, hostelName, hallName, address } = req.body;
      const coverImageURL = req.file ? req.file.path : "/uploads/default.png";
      // Ensure required fields are provided
      if (!fullName || !email || !contactNumber || !enrollmentNo || !gender || !year || !department || !residenceType) {
        return res.status(400).json({ message: "All fields are required" });
      }
      // Check if enrollment number already exists
      const existingVolunteer = await Volunteer.findOne({ enrollmentNo });
      if (existingVolunteer) {
        return res.status(400).json({ message: "Volunteer with this Enrollment No already exists" });
      }
  ...
  const handleUpdateVolunteer = async (req, res) => {
    try {
      const {email, contactNumber,year, residenceType, hostelName, hallName, address } = req.body;
  ...
  const getVolunteerById = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to retrieve volunteer", error: error.message });
    }
  };
